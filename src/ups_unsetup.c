 ...
 * ups_unsetup
 *
 * Find the instance the user has requested, and process the unsetup actions.
 *
 * Input : command line information and an output stream
 * Output: none
 * Return: none
 */
void ups_unsetup( const t_upsugo_command * const a_command_line,
		const FILE * const a_temp_file, const int a_ups_command)
{
  ...
  mproduct_list = unsetup_core(a_command_line, a_temp_file, a_ups_command);
