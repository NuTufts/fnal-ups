...
{ struct stat file_stat;
  ...
  char * toman_source=0;
  char * temp;
  int toman=0;                  /* must READ toman files to see what they are*/
  char buffer[FILENAME_MAX+1];
  if (a_db_info &&                       /* Must have database information   */
      a_db_info->config &&               /* Must have database configuration */
      a_db_info->config->man_target_dir) /* must have where to put man pages */
  { if (a_minst->table &&                /* Is there a table file            */
        a_minst->table->man_source_dir)  /* Did they specify the source      */
    { if (UPSRELATIVE(a_minst->table->man_source_dir))
      { toman_source = 
             upsutl_str_create(upsget_prod_dir(a_db_info,a_minst,0),' ');
        toman_source = upsutl_str_crecat(toman_source,"/");
        toman_source = 
             upsutl_str_crecat(toman_source,a_minst->table->man_source_dir);
      } else {
        toman_source=upsutl_str_create(a_minst->table->man_source_dir,' ');
      }
    } else {               /* man source not specificed us the default logic */
      toman_source = 
            upsutl_str_create(upsget_ups_dir(a_db_info,a_minst,0),' ');
      toman_source = upsutl_str_crecat(toman_source,"/toman");
      if (! stat(toman_source, &file_stat))           /* does ups/toman exist*/
      { if (S_ISDIR(file_stat.st_mode))               /* is it a directory   */
        { toman=1;                                    /* will have read files*/
          temp=upsutl_str_crecat(toman_source,"/man");/* a toman/man ??      */
          if (! stat(temp, &file_stat))               /* does that file exist*/
          { toman=0;                                  /* /man DON'T check fil*/
            if (S_ISDIR(file_stat.st_mode))           /* is that a directory */
            { upsmem_free(toman_source);              /* is /man free other  */              toman_source=temp;                      /* ..../toman/man      */            } else {                                  /* no good keep /toman */              upsmem_free(temp);                      /* free the test of/man*/            }
          } 
        } else {                         /* not a directory another problem  */
          upsmem_free(toman_source);
          toman_source=0;
        }
      } else {                           /* Directory doesn't exist to source*/
        upsmem_free(toman_source);
        toman_source=0;
      }
    }  
    if (toman_source)                    /* do we have a source at this point*/
    { if (!stat(a_db_info->config->man_target_dir, &file_stat)) /* exist */
      { if (S_ISDIR(file_stat.st_mode))  /* is it a directory   */
        { toman_target=
                upsutl_str_create(a_db_info->config->man_target_dir,' ');
        } else {
          /* tell them they are stupid ?? target NOT a directory */
        }
      } else {                           /* make the directory target */
        sprintf(buffer,"mkdir -p %s",a_db_info->config->man_target_dir);
        if (!system(buffer))              /* yes but i need it done today */
        { toman_target=
                upsutl_str_create(a_db_info->config->man_target_dir,' ');
    ...
    if (toman_target && toman_source) /* all sucessful so far */
    { if (toman) 
      { sprintf(buffer,"for i in `file %s/* | grep roff | awk '{ print $1 }' | sed s/://`; do; cp $i %s;done;",toman_source,toman_target);
        if (system(buffer))           
        { /* oh they will see an error ... */
        }
      } else {
        sprintf(buffer,"cp %s/* %s",toman_source,toman_target);
        if (system(buffer))           
        { /* oh they will see an error ... */
        }
      }
