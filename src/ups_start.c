...
static t_upslst_item *start_core(const t_upsugo_command * const a_command_line,
 ...
 * ups_start
 *
 * Find the instance the user has requested, and process the start actions.
 *
 * Input : command line information and an output stream
 * Output: none
 * Return: none
 */
void ups_start( const t_upsugo_command * const a_command_line,
		const FILE * const a_temp_file, const int a_ups_command)
{
  ...
  mproduct_list = start_core(a_command_line, a_temp_file, a_ups_command);
 ...
 * start_core
 *
 * Cycle through the actions for start, translate them,
